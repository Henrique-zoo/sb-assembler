...
use crate::{
    assembler::errors::{EquDirectiveSemanticErrorKind, PreprocessorError, PreprocessorErrorKind},
    assembler::interner::{Interner, Symbol},
    assembler::language::numeric_literals::{self, NumberParseError, NumericLiteral},
...
impl Preprocessor<'_> {
    ...
    pub(in crate::assembler::preprocessor) fn execute_equ_directive(
        &mut self,
        equ_decl: EquDecl,
        interner: &Interner,
    ...
    ///
    /// Encapsula o `kind` em
    /// [`crate::assembler::errors::PreprocessorErrorKind::InvalidEquDirectiveSemantic`].
    ...
    pub(in crate::assembler::preprocessor) fn replace_data_equ_use(
        &self,
        line: &mut Vec<Token>,
    ) -> Result<(), PreprocessorError> {
        let Some(Token {
