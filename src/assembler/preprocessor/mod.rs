...
//! - [`crate::assembler::preprocessor::detection`]: detectores `looks_like_*` fazem triagem
//!   permissiva. Eles decidem se uma linha parece uma tentativa de diretiva ou
//!   chamada de macro;
//! - [`crate::assembler::preprocessor::parser`]: parsers validam a forma sintática e
//!   constroem nós de IR com `NodeId`/`Span`;
//! - [`crate::assembler::preprocessor::execute`]: executores aplicam efeitos semânticos,
...
pub(in crate::assembler::preprocessor) use types::LogicalLineIter;
pub(crate) use types::{LogicalLine, PreprocessedProgram, Preprocessor};

use std::collections::HashMap;

use crate::{
    assembler::errors::{PreprocessorError, PreprocessorErrorKind},
    assembler::interner::Interner,
    assembler::language::LanguageSymbols,
    assembler::lexer::{Span, Token},
    assembler::preprocessor::{ir::NodeId, types::Section},
};
...
impl<'language> Preprocessor<'language> {
    ...
    ///   com os demais estágios para comparação por [`crate::assembler::interner::Symbol`].
    ///
    /// # Estado inicial
    /// - `macros`: vazio;
    /// - `equs`: vazio;
