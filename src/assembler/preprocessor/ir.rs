...
use crate::{
    ...
    assembler::language::numeric_literals::NumericLiteral,
    assembler::lexer::{Span, Token, TokenKind},
    assembler::preprocessor::types::Section,
};
...
pub(in crate::assembler::preprocessor) enum Sign {
    /// Prefixo `+`.
    Plus,
    /// Prefixo `-`.
    Minus,
...
/// - a diretiva não é emitida no [`crate::assembler::preprocessor::types::PreprocessedProgram`].
#[derive(Debug, Clone)]
...
///   [`crate::assembler::preprocessor::types::LogicalLine`].
#[derive(Debug, Clone)]
